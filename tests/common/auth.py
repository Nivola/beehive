...
from gibboncloudapi.common import AuthDbManager
...
class AuthManagerTestCase(CloudapiTestCase):
    ...
    def test_get_object_type(self):
        ...
        res = self.manager.get_object_type(objtype='resource', definition='container.org.group.vm')
        #self.assertEqual(res.name, name, 'Error')
    ...
    def test_remove_object_type(self):
        ...
        res = self.manager.remove_object_type(objtype='resource', definition='container.org.group.vm')
        res = self.manager.remove_object_type(objtype='resource', definition='container.org')
        self.assertEqual(res, True)
    ...
    def test_get_permission(self):
        ...
        res = self.manager.get_permission_by_object(definition='container.org')
        res = self.manager.get_permission_by_object(objid='c1.o1.g1.*', 
                                                    objtype='resource', 
    ...
    def test_add_role(self):
        name = 'role1'
        description = 'role1'
        res = self.manager.add_role(name, description)
        self.assertEqual(res, True)
    ...
    def test_add_role_bis(self):
        ...
        description = 'role1'
        with self.assertRaises(TransactionError):
            self.manager.add_role(name, description)

    def test_get_role(self):
        name = 'role1'
        res = self.manager.get_role(name)
    ...
    def test_update_role(self):
        ...
        res = self.manager.update_role(name, new_name, new_description)
        self.assertEqual(res, True)
    ...
    def test_append_role_permission(self):
        role = self.manager.get_role('role2')[0]
    ...
    def test_remove_role_permission(self):
        name = 'role2'
        role = self.manager.get_role(name)[0]
        perms1 = self.manager.get_permission_by_object(objid='c1.o1.g1.*')
        perms2 = self.manager.get_permission_by_object(objtype='service') 
    ...
    def test_add_profile(self):
        ...
        res = self.manager.add_profile(name, description)

    def test_get_profile(self):
        name = 'profile1' 
        res = self.manager.get_profile(name)

    def test_update_profile(self):
        name = 'profile1'
        new_name = 'profile2'
        new_description = 'profile1_desc'
        res = self.manager.update_profile(name, new_name=new_name, 
                                          new_description=new_description)

    def test_remove_profile(self):
        name = 'profile2'
        res = self.manager.remove_profile(name=name)

    # user
    def test_add_user(self):
        name = 'user1'
        user_type = 'user'
        profile = self.manager.get_profile('profile2')[0]
        roles = [self.manager.get_role('role2')[0]]
        active = True
        password = 'mypass'
        res = self.manager.add_user(name, user_type, roles, profile, 
    ...
    def test_update_user(self):
        name = 'user1'
        res = self.manager.update_user(name, new_name='user2', 
    ...
    def test_append_user_role(self):
        user = self.manager.get_user(name='user1')[0]
        if self.manager.add_role('role3', 'role3_desc'):
    ...
    def test_append_user_role_bis(self):
        with self.assertRaises(TransactionError):
            user = self.manager.get_user(name='user1')[0]
            role = self.manager.get_role('role3')[0]
            res = self.manager.append_user_role(user, role)
    ...
    def test_add_group(self):
        ...
        active = True
        password = 'mypass'
        res = self.manager.add_group(name, description, members, roles)
        self.assertEqual(res, True)

    def test_get_group(self):
        res = self.manager.get_group('group1')
        self.logger.debug(res)
    ...
    def test_get_user_groups(self):
        user = self.manager.get_user('user2')[0]
    ...
    def test_update_group(self):
        ...
        res = self.manager.update_group(name, new_name, new_description)
        self.assertEqual(res, True)

    def test_append_group_role(self):
        group = self.manager.get_group('group1')[0]
        if self.manager.add_role('role4', 'role4_desc'):
    ...
    def test_append_group_role_bis(self):
        with self.assertRaises(TransactionError):
            group = self.manager.get_group('group1')[0]
            role = self.manager.get_role('role4')[0]
            res = self.manager.append_group_role(group, role)
            self.assertEqual(res, False)
    ...
    def test_append_group_member(self):
        group = self.manager.get_group('group1')[0]
        if self.manager.add_user('user3', '', [], None, True, '', '', ''):
            user = self.manager.get_user('user3')[0]
