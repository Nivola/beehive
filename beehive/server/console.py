...
@cli.command()
def create_token():
    data = {"user": "admin@local", "password": "admin", "login-ip": "localhost"}
    headers = {"Content-type": "application/json"}
    res = requests.post(
        "http://localhost:8080/v1.0/nas/keyauth/token",
...
def invoke_api(method, uri, token, seckey):
    ...
    res = requests.request(method, "http://localhost:8080" + uri, headers=headers, timeout=30, verify=False)
    return res.json()
