...
class BeehiveApiClient(object):
    ...
    def __init__(self, auth_endpoints, authtype, user, pwd, secret=None, catalog_id=None, client_config=None, key=None,
                 proxy=None, prefixuri='', oauth2_grant_type='jwt'):
        ...
        self.endpoints = {'auth': [], 'event': [], 'ssh': [], 'resource': [], 'service': []}
    ...
    def __str__(self):
        try:
            ...
            client_id = None
        return '<ApiClient(%s, %s, %s, %s, %s, %s, %s, %s)>' % (self.main_endpoint, self.api_authtype, self.api_user,
                                                                self.catalog_id, client_id, self.proxy,
                                                                self.prefixuri, self.oauth2_grant_type)
    ...
    # task
    #
    def get_task_status(self, subsystem, prefix, taskid, uid=None, seckey=None, timeout=60, other_headers=None):
        """get task status
        ...
        try:
            ...
            res = self.send_request(subsystem, path, 'get', data='', uid=uid, seckey=seckey, timeout=timeout,
                                    silent=True, other_headers=other_headers)
    ...
    def get_task(self, subsystem, prefix, taskid, uid=None, seckey=None, timeout=60, other_headers=None):
        """get task
    ...
    def wait_task(self, subsystem, prefix, taskid, uid=None, seckey=None, timeout=60, delta=3, maxtime=600, trace=None,
                  other_headers=None):
        ...
        status = self.get_task_status(subsystem, prefix, taskid, uid=uid, seckey=seckey, timeout=timeout,
        ...
        # get task
        task = self.get_task(subsystem, prefix, taskid, uid=None, seckey=None, timeout=timeout,
                             other_headers=other_headers)
    ...
    def create_token(self, api_user=None, api_user_pwd=None, api_user_secret=None, other_headers=None):
        ...
        if self.api_authtype == 'keyauth':
            data = {'user': api_user, 'password': api_user_pwd}
            res = self.send_request('auth', '/v1.0/nas/keyauth/token', 'POST', data=data, other_headers=other_headers)
            ...
            uri = '%s://%s:%s%s/v1.0/nas/oauth2/token' % (endpoint['proto'], endpoint['host'], endpoint['port'], self.prefixuri)
    ...
    def exist(self, uid, other_headers=None):
        ...
        if uid is None:
            return False
        try:
            self.send_request('auth', '/v1.0/nas/tokens/%s' % uid, 'GET', data='', uid=self.uid, seckey=self.seckey,
                              silent=True, other_headers=other_headers)
            res = True
