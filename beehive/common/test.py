...
class BeehiveTestCase(unittest.TestCase):
    ...
    def post(self, uri, data=None, query=None, params=None, timeout=600, user=None, task_key='taskid'):
        if user is None:
    ...
    def put(self, uri, data=None, query=None, params=None, timeout=600, user=None, task_key='taskid'):
        if user is None:
            user = self.users[self.run_test_user]
    ...
    def patch(self, uri, data=None, query=None, params=None, timeout=600, user=None, task_key='taskid'):
        if user is None:
            user = self.users[self.run_test_user]
    ...
    def delete(self, uri, data=None, query=None, params=None, timeout=600, user=None, task_key='taskid'):
        if user is None:
            user = self.users[self.run_test_user]
    ...
    def get_task_status(self, taskid):
        try:
            res = self.call(self.module, '/v2.0/%s/worker/tasks/{oid}/status' % self.module_prefix, 'get',
                            params={'oid': taskid}, runlog=False, **self.users[self.run_test_user])
            job = res.get('task_instance')
            state = job.get('status')
    ...
    def wait_task(self, taskid, delta=3, accepted_state='SUCCESS', maxtime=600):
        ...
        while state not in ['SUCCESS', 'FAILURE', 'TOKEN_INVALID']:
            ...
            elapsed += delta
            if elapsed > maxtime and state != accepted_state:
                self.runlogger.info('...TIMEOUT - elapsed %s' % elapsed)
                state = TIMEOUT

        logger.info('wait_task - state: %s' % state)
        self.runlogger.info('state: %s' % state)
        if state != 'TOKEN_INVALID':
            self.assertEqual(state, accepted_state)
