...
def start_task_manager(params):
    ...
    frmt = "%(asctime)s %(levelname)s %(process)s:%(thread)s %(api_id)s::%(task_name)s:%(task_id)s:%(step_name)s" \
           " %(name)s:%(funcName)s:%(lineno)d | %(message)s"
    LoggerHelper.simple_handler(main_loggers, logger_level, frmt=frmt, formatter=ExtTaskFormatter, handler=K8shHandler)
...
def start_scheduler(params):
    ...
    LoggerHelper.simple_handler(loggers, logger_level, frmt=frmt, formatter=ExtTaskFormatter, handler=K8shHandler)
...
def configure_batch_task_manager(params):
    ...
    frmt = '[%(asctime)s: %(levelname)s/%(processName)s] [%(task_name)s:%(task_id)s] %(name)s:%(funcName)s:%(lineno)d' \
           ' - %(message)s'
    ...
    if api_manager.elasticsearch is not None:
        ...
        LoggerHelper.elastic_handler(main_loggers, logger_level, api_manager.elasticsearch,
