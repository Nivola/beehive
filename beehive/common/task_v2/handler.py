...
class TaskResult(object):
    ...
    def elapsed(self):
        if isinstance(self.start_time, datetime) and isinstance(self.stop_time, datetime):
            return round((self.stop_time - self.start_time) / timedelta(microseconds=1)/1000000, 3)
        return 0
    ...
    def step_success(self, task_id, step_id, result):
        ...
        entity_id = self.manager.update_entity(SchedulerStep, uuid=step_id, status=SchedulerState.SUCCESS,
                                               result=result, stop_time=stop_time, run_time=stop_time)
    ...
    def step_failure(self, task_id, step_id, error):
        ...
        self.stop_time = stop_time
        entity = self.manager.get_entity(SchedulerStep, step_id)
        entity_id = self.manager.update_entity(SchedulerStep, uuid=step_id, status=SchedulerState.FAILURE,
                                               stop_time=stop_time, result=False)
        logger.debug2('update task step %s record' % entity_id)
    ...
    def task_prerun(self, **args):
        ...
        # update already created task
            ...
            status = SchedulerState.PENDING
            entity = self.manager.update_entity(SchedulerTask, uuid=task_id, name=task.name, status=status,
    ...
    def task_start(self, task):
        ...
        entity = self.manager.update_entity(SchedulerTask, uuid=task_id, name=task.name, worker=task.hostname,
                                            objid=task.objid, objtype=task.objtype, objdef=task.objdef, args=vargs,
                                            kwargs=kwargs, status=status, api_id=task.api_id, parent=task.parent)
        logger.debug2('update db task %s record' % entity)
