...
class BaseAuthController(ApiController):
    ...
    def set_identity(self, uid, identity, expire=True, expire_time=None):
        ...
        """
        try:
            dbgprint(uid=uid, identity=identity)
            IdentityMgr.set_identity(
                uid, identity, self.module.redis_identity_manager, expire=expire, expire_time=expire_time
            )
    ...
    def get_identities(self):
        ...
        try:
            self.logger.debug("Getting identities from redis")
            return IdentityMgr.get_identities(self.module.redis_identity_manager)
    ...
    def base_login(self, name, domain, password, login_ip, dbuser, dbuser_attribs):
        ...
        # login user
        try:
            user = self.module.authentication_manager.login(
                dbuser.name, self.get_login_email(dbuser), password, domain, login_ip
            )
        except AuthError as ex:
    ...
    def __set_user_perms(self, dbuser, user):
        ...
        :return:
        """
        authDbManager: AuthDbManager = self.auth_manager
        perms = authDbManager.get_login_permissions(dbuser)
    ...
    def simple_http_login(self, name, domain, password, login_ip):
        ...
        # login user
        try:
            user, attrib = self.base_login(
                self.get_login_email(dbuser),
                domain,
                password,
                login_ip,
                dbuser,
