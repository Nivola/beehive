...
class CustomTestResult(unittest.result.TestResult):
    ...
    def __init__(self, stream, descriptions: bool, verbosity: int, report_file: str, test_group: str, test_filename: str):
    ...
    def writeLineToFile(self):
        if self.test_group is None:
            line = '%s - %s - %s' % (self.test_filename, self.testInfo.desc, self.testInfo.esito)
        else:
            line = '%s - %s - %s - %s' % (self.test_filename, self.test_group, self.testInfo.desc, self.testInfo.esito)
        # a = append, w = write
...
class CustomTestRunner(object):
    ...
    def __init__(self, report_file, test_filename, stream=None, descriptions=True, verbosity=1,
    ...
    def _makeResult(self):
        return self.resultclass(self.stream, self.descriptions, self.verbosity, self.report_file, self.test_group, self.test_filename)
